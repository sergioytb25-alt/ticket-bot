  ...
  async execute(interaction) {
    ...
    if (!isValidHexColor(color)) {
      return interaction.reply({
        content: '❌ Format invalide! Utilisez le format #RRGGBB (ex: #FEE75C)',
        ephemeral: true
      });
    }

    config.colors.warning = color;
    saveConfig(config);

    interaction.reply({
      content: `✅ Couleur d'avertissement modifiée en ${color}!`,
      ephemeral: true
    });
  }
};