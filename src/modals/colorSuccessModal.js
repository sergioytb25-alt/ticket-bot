  ...
  async execute(interaction) {
    ...
    if (!isValidHexColor(color)) {
      return interaction.reply({
        content: '❌ Format invalide! Utilisez le format #RRGGBB (ex: #57F287)',
        ephemeral: true
      });
    }

    config.colors.success = color;
    saveConfig(config);

    interaction.reply({
      content: `✅ Couleur de succès modifiée en ${color}!`,
      ephemeral: true
    });
  }
};